        ...
        <div className="min-h-screen bg-surface pt-6 pb-24">
            <div className="space-y-8 px-4 md:px-8 max-w-2xl md:mx-auto">
                ...
                <Section title="Language & Audio">
                    <SettingButton icon={Globe} iconBg="bg-cyan-500/20" iconColor="text-cyan-400" label="Audio Language" description={getLanguageName(settings.language)} onClick={() => setShowLanguageModal(true)} />
                    <SettingButton icon={Subtitles} iconBg="bg-orange-500/20" iconColor="text-orange-400" label="Subtitles" description={getSubtitleName(settings.subtitle_language)} onClick={() => setShowSubtitleModal(true)} border />
                </Section>

                {/* Playback */}
                <Section title="Playback">
                    <SettingToggle icon={Play} iconBg="bg-brand/20" iconColor="text-brand" label="Auto-play" description={settings.auto_play ? 'Videos start automatically' : 'Videos start paused'} value={settings.auto_play} onChange={(v) => updateSetting('auto_play', v)} />
                    <SettingButton icon={Volume2} iconBg="bg-green-500/20" iconColor="text-green-400" label="Video Quality" description={qualityLabels[settings.default_quality]} onClick={() => setShowQualityModal(true)} border />
                    <SettingToggle icon={Bell} iconBg="bg-yellow-500/20" iconColor="text-yellow-400" label="Notifications" description={settings.notifications ? 'Enabled' : 'Disabled'} value={settings.notifications} onChange={(v) => updateSetting('notifications', v)} border />
                </Section>
            ...
            <Modal isOpen={showQualityModal} onClose={() => setShowQualityModal(false)} title="Video Quality">
                <div className="space-y-2">
                    {(['auto', '1080p', '720p', '480p'] as Quality[]).map(q => (
                        <ModalOption key={q} label={qualityLabels[q]} selected={settings.default_quality === q} onClick={() => { updateSetting('default_quality', q); setShowQualityModal(false) }} />
                    ))}
                </div>
            </Modal>
            ...
            <Modal isOpen={showLanguageModal} onClose={() => setShowLanguageModal(false)} title="Audio Language">
                <div className="space-y-2">
                    {languages.map(lang => (
                        <ModalOption key={lang.code} label={lang.name} selected={settings.language === lang.code} onClick={() => { updateSetting('language', lang.code); setShowLanguageModal(false) }} />
                    ))}
                </div>
            </Modal>
            ...
            <Modal isOpen={showSubtitleModal} onClose={() => setShowSubtitleModal(false)} title="Subtitles">
                <div className="space-y-2">
                    {subtitleLanguages.map(lang => (
                        <ModalOption key={lang.code} label={lang.name} selected={settings.subtitle_language === lang.code} onClick={() => { updateSetting('subtitle_language', lang.code); setShowSubtitleModal(false) }} />
                    ))}
                </div>
            </Modal>
